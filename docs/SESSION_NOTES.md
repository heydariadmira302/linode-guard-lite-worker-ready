...
- Server 2025 DD 默认使用 `https://dl.lamp.sh/vhd/zh-cn_win2025.xz`；Win11 DD 默认使用 `https://dl.lamp.sh/vhd/zh-cn_windows11_22h2.xz`，无需配置环境变量。
...
- Server 2025 DD 改为默认使用 `https://dl.lamp.sh/vhd/zh-cn_win2025.xz`。
- 新增 Win11 DD 版本 `w11-cn-dd`：默认使用 `https://dl.lamp.sh/vhd/zh-cn_windows11_22h2.xz`。
...
## 2026-06-02 移除 DD 镜像环境变量覆盖

- 用户确认：`WINDOWS_2025_CN_DD_IMAGE_URL` 与 `WINDOWS_11_CN_DD_IMAGE_URL` 不需要存在，DD 快速安装应完全开箱即用。
- 移除 Worker Env 中两个 DD 镜像 URL 变量，服务层只使用内置镜像：`https://dl.lamp.sh/vhd/zh-cn_win2025.xz` 与 `https://dl.lamp.sh/vhd/zh-cn_windows11_22h2.xz`。
- 文档同步删除环境变量覆盖说明，避免一键部署用户误以为还需要额外配置。
