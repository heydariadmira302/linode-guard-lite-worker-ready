...
3. Cloudflare Cron Trigger：建议每 5 分钟触发一次，用于统一执行 Job Runner。
...
{
  "enabled": true,
  "ip_allowlist": ["1.2.3.4", "5.6.7.0/24"],
...
setup 不应该：自动创建 D1 数据库、自动设置 Worker Secret、自动修改 Cloudflare 配置、自动添加作者个人配置。
...
缓解：默认日志保留 1 天；批量操作控制并发；Job Runner 每 5 分钟执行；失败写 job_runs 便于排查。
...
验收：Cron 每 5 分钟触发；每个 job 有运行记录；一个 job 失败不影响其他 job；diagnostics/jobs 可展示最近状态。
