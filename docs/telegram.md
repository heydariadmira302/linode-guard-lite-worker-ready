...
服务器管理页提供「🪟 创建 Windows 服务器」。当前开放 Windows Server 2022 稳定路线、Windows Server 2025 简体中文版 / English 实验路线和 Windows 11 Enterprise LTSC 2024 实验路线：
...
Windows 创建成功页只表示 Linode 已创建并开始运行 StackScript。安装真正完成后，Windows 首次登录阶段会启用 RDP 并调用安装完成回调；Bot 收到有效一次性 token 后会主动发送“Windows 安装完成，可以尝试远程桌面登录”通知，包含服务器名称、实例 ID、RDP 地址和用户名 `Administrator`。Bot 不会重复发送一次性密码，请使用创建成功页保存的密码。
