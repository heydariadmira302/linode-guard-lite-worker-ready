...
Telegram 不直接实现 Windows 创建业务逻辑，只保存会话状态、展示选项、收集确认并调用 service/API。流程为：选择版本 → Win11 选择语言 zh-cn/en-us（Server 2025 按版本固定 zh-cn 或 en-us） → 选择凭据（自动生成强密码或自己输入密码）→ 设置 Linode 实例名称（可跳过自动命名）→ Region → Plan → Firewall → 高危确认。Win11 页面明确提示 Bot 会自动查找官方 ISO，不需要用户输入 ISO URL。DD 快速安装路线默认参考 `https://dl.lamp.sh/vhd/` 内置镜像源，无需额外配置。创建成功后会显示一次性 Windows 登录用户名、Windows 密码和临时 Ubuntu root 密码；默认用户名为 `Administrator`。Server 2022 预计 15-30 分钟，Server 2025 DD 预计 5-15 分钟，Win11 预计 20-40 分钟，完成后用 RDP 3389 连接。

DD 快速安装参考 bin456789/reinstall 与 leitbogioro/Tools 的思路：下载 DD 镜像、写入目标磁盘、挂载 Windows 分区并注入首次启动脚本。当前开放 Windows Server 2025 与 Windows 11 简体中文实验路线，不使用公开默认密码；默认镜像参考 leitbogioro/Tools 提到的 TeddySun 镜像源 `dl.lamp.sh/vhd`。后续如扩展更多 DD 镜像，也必须先落 service/API，并保留高危确认、审计日志和一次性密码展示规则。
