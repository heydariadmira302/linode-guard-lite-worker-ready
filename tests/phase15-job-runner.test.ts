...
type Schedule = { id: number; name: string; enabled: number; action: string; scope: string; account_id: number | null; cron_expr: string; timezone: string; last_run_at: string | null; next_run_at: string | null; created_at: string; updated_at: string; deleted_at: string | null; metadata_json: string | null };
...
async function addAccount(db:FakeD1Database){db.accounts.push({id:1,alias:"default",encrypted_token:await encryptLinodeToken("token-default","encryption-key"),token_fingerprint:"fp_1",token_status:"valid",status:"active",created_at:"2026-01-01T00:00:00.000Z",updated_at:"2026-01-01T00:00:00.000Z",deleted_at:null,last_seen_login_id:null,last_login_check_at:null})}
function instanceList(){return {data:[{id:101,label:"vm-101",status:"running",region:"jp-osa",type:"g6-standard-1"}]}}
...
describe("Phase 15 job runner",()=>{
 it("Cloudflare scheduled handler runs due schedule_power and checkin_monitor jobs, records runs, and never leaks tokens", async()=>{
  ...
  try{
   ...
   expect(db.presenceRuns.length).toBe(0);
   ...
   expect(calls).toEqual(expect.arrayContaining(["GET https://api.linode.com/v4/account/logins Bearer token-default", "GET https://api.linode.com/v4/linode/instances Bearer token-default", "POST https://api.linode.com/v4/linode/instances/101/shutdown Bearer token-default"]));
	   expect(calls.some((call)=>call.startsWith("POST https://api.telegram.org/botbot-token/sendMessage"))).toBe(true);
