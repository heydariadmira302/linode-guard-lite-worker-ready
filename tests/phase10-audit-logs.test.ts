...
describe("Phase 10 audit logs API and Telegram menu", () => {
  it("lists audit logs via authenticated HTTP API with limit/offset and never returns sensitive metadata", async () => {
    ...
    expect(body.data.audit_logs[0]).toMatchObject({ action_label: "删除服务器", risk_level_label: "严重", result_label: "失败", error_message: "Linode Token 无效" });
  ...
  it("shows recent audit logs from Telegram menu", async () => {
    ...
    expect(text).toContain("用途：记录谁在什么时间做了什么操作");
    expect(text).toContain("操作：删除服务器");
    expect(text).toContain("对象：服务器 102");
    expect(text).toContain("风险：严重");
    expect(text).toContain("结果：失败");
    expect(text).toContain("失败原因：Linode Token 无效");
    expect(text).toContain("请求编号：req_2");
