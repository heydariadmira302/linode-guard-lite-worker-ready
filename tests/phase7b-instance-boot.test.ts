...
describe("Phase 7B single instance boot", () => {
  it("boots one instance via authenticated HTTP API, calls Linode POST boot, and writes audit log", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      expect(String(input)).toBe("https://api.linode.com/v4/linode/instances/101/boot");
      expect(init?.method).toBe("POST");
      expect(new Headers(init?.headers).get("authorization")).toBe("Bearer token-default");
      return new Response(null, { status: 200 });
    });
    try {
      const response = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101/boot", { method: "POST" }), env as never);
      ...
      expect(body.data).toMatchObject({ action: "boot", instance_id: 101, result: "success" });
      ...
      expect(db.auditLogs).toEqual(expect.arrayContaining([
        expect.objectContaining({ action: "instance.boot", target_type: "instance", target_id: "101", risk_level: "medium", result: "success" })
      ]));
    } finally {
  ...
  it("returns unified errors and failed audit log for boot failures", async () => {
    const db = new FakeD1Database();
    await addAccount(db, { id: 1, alias: "default", token: "token-default" });
    await addAccount(db, { id: 2, alias: "deleted", token: "token-deleted", status: "deleted" });
    const env = { ...baseEnv, DB: db as unknown as D1Database };

    const unauthorized = await worker.fetch(new Request("https://example.com/api/v1/accounts/1/instances/101/boot", { method: "POST" }), env as never);
    expect(unauthorized.status).toBe(401);
    expect(((await unauthorized.json()) as { error: { code: string } }).error.code).toBe("UNAUTHORIZED");

    const missing = await worker.fetch(apiRequest("/api/v1/accounts/2/instances/101/boot", { method: "POST" }), env as never);
    ...
    try {
      const invalid = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101/boot", { method: "POST" }), env as never);
      expect(invalid.status).toBe(401);
      expect(((await invalid.json()) as { error: { code: string } }).error.code).toBe("TOKEN_INVALID");

      const permission = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101/boot", { method: "POST" }), env as never);
      ...
      const apiError = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101/boot", { method: "POST" }), env as never);
      expect(apiError.status).toBe(502);
      expect(((await apiError.json()) as { error: { code: string } }).error.code).toBe("LINODE_API_ERROR");

      expect(db.auditLogs).toEqual(expect.arrayContaining([
        expect.objectContaining({ action: "instance.boot", result: "failed", error_code: "TOKEN_INVALID" }),
        expect.objectContaining({ action: "instance.boot", result: "failed", error_code: "TOKEN_PERMISSION_ERROR" }),
        expect.objectContaining({ action: "instance.boot", result: "failed", error_code: "LINODE_API_ERROR" })
      ]));
    } finally {
      fetchMock.mockRestore();
    }
  ...
  it("shows a minimal Telegram boot button on instance detail and can boot one instance without adding other write operations", async () => {
    const db = new FakeD1Database();
    await addAccount(db, { id: 1, alias: "default", token: "token-default" });
    const env = { ...baseEnv, DB: db as unknown as D1Database };
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      if (String(input).endsWith("/linode/instances/101/boot")) {
        expect(init?.method).toBe("POST");
        return new Response(null, { status: 200 });
      }
      return new Response(JSON.stringify({ id: 101, label: "web-1", status: "offline", region: "jp-osa", type: "g6-standard-1" }), { status: 200 });
    });
    try {
      ...
      expect(detailKeyboard).toEqual(expect.arrayContaining([{ text: "✅ 开机", callback_data: "instances:boot:1:101:account_1", style: "success" }]));
      ...
      const bootResponse = await worker.fetch(telegramRequest(callbackUpdate("instances:boot:1:101:account_1")), env as never);
      const bootBody = await bootResponse.json() as { data: { telegram: { payload: { text: string; reply_markup: { inline_keyboard: Array<Array<{ text: string; callback_data: string }>> } } } } };
      const rawBoot = JSON.stringify(bootBody);
      expect(bootResponse.status).toBe(200);
      expect(bootBody.data.telegram.payload.text).toContain("结果：已成功提交");
      expect(bootBody.data.telegram.payload.text).toContain("Linode API 已接受请求");
      expect(bootBody.data.telegram.payload.text).toContain("#101");
      expect(rawBoot).toContain("刷新服务器状态");
      expect(rawBoot).not.toContain("⚠️ 关机");
      expect(rawBoot).not.toContain("批量");
    } finally {
      fetchMock.mockRestore();
    }
  });
