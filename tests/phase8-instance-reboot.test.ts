...
describe("Phase 8 single instance reboot", () => {
  ...
  it("shows a minimal Telegram reboot button on instance detail and can reboot one instance without adding delete or batch operations", async () => {
    ...
    try {
      ...
      const rebootResponse = await worker.fetch(telegramRequest(callbackUpdate("instances:reboot:1:101")), env as never);
      ...
      expect(rebootBody.data.telegram.payload.text).toContain("重启请求已发送");
