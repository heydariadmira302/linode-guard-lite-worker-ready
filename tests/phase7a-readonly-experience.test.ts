...
describe("Phase 7A read-only instance experience and docs", () => {
  it("renders richer Telegram instance detail fields while keeping the flow read-only", async () => {
    ...
    try {
      ...
      expect(body.data.telegram.payload.text).toContain("• 更新：2026-01-02T00:00:00");
      ...
      expect(raw).not.toContain("menu:batch");
      expect(raw).not.toContain("批量操作");
    } finally {
  ...
  it("documents Phase 6 read-only API and Telegram flow for Phase 7A", async () => {
    ...
    expect(telegramDoc).toContain("服务器管理入口采用“列表优先”");
    expect(telegramDoc).toContain("🔎 筛选");
    expect(telegramDoc).toContain("👤 按账号查看");
