...
type AccountRecord = { id: number; alias: string; group_id: number; token_status: string; status: string };
...
describe("Phase 19 groups", () => {
  ...
  it("supports Telegram group list, create, rename, delete, and account list flows", async () => {
    ...
    expect(menuBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([{ text: "新建分组", callback_data: "groups:create" }]));
    ...
    expect(detailBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([
      { text: "查看账号", callback_data: "groups:accounts:2" },
      { text: "查看服务器", callback_data: "groups:instances:2" },
      { text: "重命名", callback_data: "groups:rename:2" },
      { text: "删除空分组", callback_data: "groups:delete_confirm:2" }
    ]));
    ...
    expect(emptyInstancesBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toContainEqual({ text: "返回分组详情", callback_data: "groups:detail:3" });
  ...
  it("returns to account add group selection after creating a group from the account flow", async () => {
    const db = new FakeD1Database();
    const env = { ...baseEnv, DB: db as unknown as D1Database };

    await worker.fetch(telegramRequest(callbackUpdate("accounts:add")), env as never);
    await worker.fetch(telegramRequest(messageUpdate("洛杉矶主号", 41)), env as never);
    await worker.fetch(telegramRequest(callbackUpdate("accounts:add:group_create")), env as never);
    const created = await worker.fetch(telegramRequest(messageUpdate("洛杉矶", 42)), env as never);
    const body = await created.json() as { data: { telegram: { payload: { text: string; reply_markup: { inline_keyboard: Array<Array<{ text: string; callback_data: string }>> } } } } };

    expect(body.data.telegram.payload.text).toContain("分组已创建");
    expect(body.data.telegram.payload.text).toContain("请选择账号所属分组");
    expect(body.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([
      { text: "洛杉矶", callback_data: "accounts:add:group:2" }
    ]));
  });
});
