...
describe("Phase 9 single instance delete", () => {
  it("deletes one instance via authenticated HTTP API, calls Linode DELETE, and writes critical audit log", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      expect(String(input)).toBe("https://api.linode.com/v4/linode/instances/101");
      expect(init?.method).toBe("DELETE");
      expect(new Headers(init?.headers).get("authorization")).toBe("Bearer token-default");
      return new Response(null, { status: 200 });
    });
    try {
      const response = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101", { method: "DELETE" }), env as never);
      ...
      expect(body.data).toMatchObject({ action: "delete", instance_id: 101, result: "success" });
      ...
      expect(db.auditLogs).toEqual(expect.arrayContaining([
        expect.objectContaining({ action: "instance.delete", target_type: "instance", target_id: "101", risk_level: "critical", result: "success" })
      ]));
    } finally {
  ...
  it("returns unified errors and failed critical audit log for delete failures", async () => {
    const db = new FakeD1Database();
    await addAccount(db, { id: 1, alias: "default", token: "token-default" });
    await addAccount(db, { id: 2, alias: "deleted", token: "token-deleted", status: "deleted" });
    const env = { ...baseEnv, DB: db as unknown as D1Database };

    const unauthorized = await worker.fetch(new Request("https://example.com/api/v1/accounts/1/instances/101", { method: "DELETE" }), env as never);
    expect(unauthorized.status).toBe(401);
    expect(((await unauthorized.json()) as { error: { code: string } }).error.code).toBe("UNAUTHORIZED");

    const missing = await worker.fetch(apiRequest("/api/v1/accounts/2/instances/101", { method: "DELETE" }), env as never);
    ...
    try {
      const invalid = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101", { method: "DELETE" }), env as never);
      expect(invalid.status).toBe(401);
      expect(((await invalid.json()) as { error: { code: string } }).error.code).toBe("TOKEN_INVALID");

      const permission = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101", { method: "DELETE" }), env as never);
      ...
      const apiError = await worker.fetch(apiRequest("/api/v1/accounts/1/instances/101", { method: "DELETE" }), env as never);
      expect(apiError.status).toBe(502);
      expect(((await apiError.json()) as { error: { code: string } }).error.code).toBe("LINODE_API_ERROR");

      expect(db.auditLogs).toEqual(expect.arrayContaining([
        expect.objectContaining({ action: "instance.delete", risk_level: "critical", result: "failed", error_code: "TOKEN_INVALID" }),
        expect.objectContaining({ action: "instance.delete", risk_level: "critical", result: "failed", error_code: "TOKEN_PERMISSION_ERROR" }),
        expect.objectContaining({ action: "instance.delete", risk_level: "critical", result: "failed", error_code: "LINODE_API_ERROR" })
      ]));
    } finally {
      fetchMock.mockRestore();
    }
  ...
  it("shows Telegram delete confirmation before deleting one instance", async () => {
    ...
    const env = { ...baseEnv, DB: db as unknown as D1Database };
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      if (String(input) === "https://api.linode.com/v4/linode/instances/101" && init?.method === "DELETE") {
        return new Response(null, { status: 200 });
      }
    ...
    try {
      ...
      expect(detailKeyboard).toEqual(expect.arrayContaining([{ text: "🔄 重启", callback_data: "instances:confirm_reboot:1:101:account_1", style: "primary" }]));

      const shutdownConfirmResponse = await worker.fetch(telegramRequest(callbackUpdate("instances:confirm_shutdown:1:101:account_1")), env as never);
      const shutdownConfirmBody = await shutdownConfirmResponse.json() as { data: { telegram: { payload: { text: string; reply_markup: { inline_keyboard: Array<Array<{ text: string; callback_data: string }>> } } } } };
      expect(shutdownConfirmBody.data.telegram.payload.text).toContain("确认关机服务器");
      expect(shutdownConfirmBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([
        { text: "⚠️ 确认关机", callback_data: "instances:shutdown:1:101:account_1", style: "primary" },
        { text: "❌ 取消，返回详情", callback_data: "instances:detail:1:101:account_1" }
      ]));
      expect(detailKeyboard).toEqual(expect.arrayContaining([{ text: "🚨 危险操作", callback_data: "instances:danger:1:101:account_1", style: "danger" }]));
      expect(detailKeyboard).not.toContainEqual({ text: "删除这台服务器", callback_data: "i:cd:1:101" });
      expect(rawDetail).not.toContain("menu:batch");
      expect(rawDetail).not.toContain("批量操作");

      const moreResponse = await worker.fetch(telegramRequest(callbackUpdate("instances:danger:1:101:account_1")), env as never);
      const moreBody = await moreResponse.json() as { data: { telegram: { payload: { text: string; reply_markup: { inline_keyboard: Array<Array<{ text: string; callback_data: string }>> } } } } };
      expect(moreBody.data.telegram.payload.text).toContain("危险操作");
      expect(moreBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([
        { text: "🚨 删除服务器", callback_data: "i:cd:1:101", style: "danger" },
        { text: "❌ 取消，返回详情", callback_data: "instances:detail:1:101:account_1" }
      ]));

      const dangerResponse = await worker.fetch(telegramRequest(callbackUpdate("instances:danger:1:101:account_1")), env as never);
      const dangerBody = await dangerResponse.json() as { data: { telegram: { payload: { text: string; reply_markup: { inline_keyboard: Array<Array<{ text: string; callback_data: string }>> } } } } };
      expect(dangerBody.data.telegram.payload.text).toContain("危险操作");
      expect(dangerBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([
        { text: "🚨 删除服务器", callback_data: "i:cd:1:101", style: "danger" },
        { text: "❌ 取消，返回详情", callback_data: "instances:detail:1:101:account_1" }
      ]));

      const confirmResponse = await worker.fetch(telegramRequest(callbackUpdate("i:cd:1:101")), env as never);
      ...
      expect(confirmBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([
        { text: "🚨 确认删除", callback_data: "instances:delete:1:101", style: "danger" },
        { text: "❌ 取消，返回详情", callback_data: "instances:detail:1:101" }
      ]));
      ...
      const rawDelete = JSON.stringify(deleteBody);
      expect(deleteResponse.status).toBe(200);
      expect(deleteBody.data.telegram.payload.text).toContain("结果：已成功提交");
      expect(deleteBody.data.telegram.payload.text).toContain("#101");
      expect(deleteBody.data.telegram.payload.text).toContain("Linode API 已接受删除请求");
      expect(deleteBody.data.telegram.payload.reply_markup.inline_keyboard.flat()).toEqual(expect.arrayContaining([{ text: "📄 查看审计日志", callback_data: "menu:audit_logs" }]));
      ...
      expect(rawDelete).not.toContain("批量");
    } finally {
      fetchMock.mockRestore();
    }
  });
