...
export function renderSecurityCheckResultText(result: SecurityCheckResult): string {
  const lines = [
    "账号安全事件检查结果",
    "",
    `结果：${formatSecurityCheckResult(result.result)}`,
  ...
  if (failed.length > 0) {
    lines.push("", "失败账号：");
    for (const item of failed.slice(0, 10)) lines.push(`#${item.account_id} ${item.account_alias}：${formatSecurityEventType(item.error_code ?? "UNKNOWN")}`);
  }
  return lines.join("\n");
}

export function renderSecurityCheckResultKeyboard(): TelegramInlineKeyboardMarkup {
  return { inline_keyboard: [[{ text: "查看未确认", callback_data: "security:events:open" }], [{ text: "❤️ 打卡", callback_data: "admin_presence:checkin" }]] };
}
...
export function renderSecurityEventStatusUpdateText(event: SecurityEventRecord): string {
  const lines = ["✅ 安全事件已更新", "", `事件：#${event.id}`, `状态：${formatSecurityEventStatus(event.status)}`];
