...
export class SecurityEventsRepository {
  ...
  async createSecurityEvent(input: SecurityEventInput): Promise<SecurityEventRecord> {
    const result = await this.db.prepare(`INSERT INTO security_events
      (account_id, type, severity, status, login_event_id, linode_login_id, username, ip, country, region, city, occurred_at, metadata_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(input.account_id ?? null, input.type, input.severity, input.status ?? "open", input.login_event_id ?? null, input.linode_login_id ?? null, input.username ?? null, input.ip ?? null, input.country ?? null, input.region ?? null, input.city ?? null, input.occurred_at, input.metadata_json ?? null)
      .run();
    const id = Number(result.meta.last_row_id);
  ...
  async findRecentSecurityEvent(params: { account_id: number; type: string; since: string }): Promise<SecurityEventRecord | null> {
    return await this.db.prepare(`SELECT id, account_id, type, severity, status, login_event_id, linode_login_id, username, ip, country, region, city, occurred_at, created_at, updated_at
      FROM security_events
      WHERE account_id = ? AND type = ? AND occurred_at >= ?
      ORDER BY occurred_at DESC, id DESC
      LIMIT 1`).bind(params.account_id, params.type, params.since).first<SecurityEventRecord>();
  }
