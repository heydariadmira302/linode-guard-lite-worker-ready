...
export class GroupsRepository {
  ...
  async list(): Promise<GroupWithCountsRecord[]> {
    ...
    const result = await this.db.prepare(`SELECT g.id, g.name, g.is_default, g.created_at, g.updated_at, g.deleted_at,
      ...
      FROM groups g
      LEFT JOIN linode_accounts a ON a.group_id = g.id AND COALESCE(a.status, 'active') = 'active'
  ...
  async countActiveAccounts(id: number): Promise<number> {
    const row = await this.db.prepare(`SELECT COUNT(*) AS count FROM linode_accounts WHERE group_id = ? AND COALESCE(status, 'active') = 'active'`).bind(id).first<{ count: number }>();
    return Number(row?.count ?? 0);
  }

  async moveAccount(accountId: number, groupId: number): Promise<void> {
    await this.db.prepare(`UPDATE linode_accounts SET group_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND COALESCE(status, 'active') = 'active'`).bind(groupId, accountId).run();
  }
}
