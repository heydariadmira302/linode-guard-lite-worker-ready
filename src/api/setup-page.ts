...
<body>
  ...
  <p class="muted">页面版本：one-click-setup-v5（默认不展示运行时密钥，降低初始化泄露风险）</p>
...
function boot() {
  ...
  result.textContent = '页面已加载，可以开始安装。版本：one-click-setup-v5。默认不会展示运行时密钥。';
}
...
function formatInstallResult(response) {
  ...
  return [
    '✅ 一键安装完成',
    '',
    '运行时密钥已生成或已存在。为了安全，安装页面默认不会展示 API_AUTH_TOKEN、TELEGRAM_WEBHOOK_SECRET 或 LINODE_TOKEN_ENCRYPTION_KEY。',
    '如需查看 API_AUTH_TOKEN，请通过 Cloudflare Worker Secrets 或受控的 D1 控制台查询；不要截图或发到聊天里。',
    '',
