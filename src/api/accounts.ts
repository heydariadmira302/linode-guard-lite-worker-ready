...
export async function handleCreateAccount(request: Request, env: Env, requestId: string): Promise<Response> {
  ensureDb(env, requestId);
  const body = await readJsonBody(request, requestId) as { alias?: unknown; token?: unknown; group_id?: unknown };
  ...
  const account = await new AccountService(env).createAccount(
    { alias: body.alias, token: body.token, group_id: normalizeOptionalGroupId(body.group_id, requestId) },
...
function normalizeOptionalGroupId(value: unknown, requestId: string): number | null | undefined {
  if (value === undefined) return undefined;
  if (value === null || value === "") return null;
  const groupId = Number(value);
  if (!Number.isInteger(groupId) || groupId <= 0) {
    throw new AppError(ErrorCode.VALIDATION_ERROR, "Invalid group_id", requestId, 400);
  }
  return groupId;
}
