...
export async function handleSetupInitialize(request: Request, env: Env, requestId: string): Promise<Response> {
  ...
  const data = await new SetupService(env).initializeDefaults({ manualSecrets: body.runtime_secrets ?? {}, configureTelegramWebhook: body.configure_telegram_webhook === true, webhookUrl, revealSecrets: body.reveal_runtime_secrets === true });
  return createJsonResponse({ ok: true, data }, { requestId });
}

type SetupInitializeBody = { runtime_secrets?: Partial<RuntimeSecrets>; configure_telegram_webhook?: boolean; reveal_runtime_secrets?: boolean };
